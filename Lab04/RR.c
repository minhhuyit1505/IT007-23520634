...
int main() {
    ...
    while (iTerminated < iNumberOfProcess) {
        ...
        if (iReady > 0) {
            ...
            int execTime = (ReadyQueue[0].iBurst > QTime) ? QTime : ReadyQueue[0].iBurst;
            pushProcess(&iGantt, Gantt, ReadyQueue[0]);
            Gantt[iGantt - 1].iBurst = execTime;
            Gantt[iGantt - 1].iFinish = curTime + execTime;

            if (execTime == ReadyQueue[0].iBurst) {
                ReadyQueue[0].iStart = iStart[ReadyQueue[0].iPID];
                ReadyQueue[0].iFinish = curTime + execTime;
    ...
    for (int i = 1; i < iGantt; i++) {
        if (Gantt[pos].iPID == Gantt[i].iPID) {
            Gantt[pos].iBurst += Gantt[i].iBurst;
